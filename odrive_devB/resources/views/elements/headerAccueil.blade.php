...
{{--10.08.2023--}}
<header >
    <div class="header-top pt-10 pt-lg-10 pt-md-10" style="background-color: #ffffff">
        <div class="container-fluid">
            <div class="row">
                <div class="col-lg-12 d-flex justify-content-start col-md-4 col-sm-12 col-xs-12">
                    <div class="col-lg-4 align-items-center">
                        <a href="{{route('/')}}">
                            <img src="{{$theme->getLogo()}}" class="img-fluid d-inline" style="max-height: 80px;">
                        </a>
                    </div>
                    <div class="col-lg-6 text-right col-md-4 col-sm-12 col-xs-12">
                        <div class="header-top-menu">
                            <ul >
                                ...
                                @endif
                                <li><a class="section" href="#section-patner">Devenir patenaire <i class="fa fa-handshake-o"></i></a></li> {{--Patner--}}
                                <li><a href="{{route('/account')}}">{{$lang->get(41)}} <i class="fa fa-user"></i></a></li> {{--My account--}}
                                <li><a href="{{route('/wishlist')}}">{{$lang->get(42)}} <i class="fa fa-heart fa-beat" style="color: red; --fa-animation-duration: 2s;"></i></a></li> {{--Wishlist--}}
                                @if (Auth::check())
                    ...
                    <div class="col-lg-2 text-center col-md-4 col-sm-12 col-xs-12">
                        <div class="lang-currency-dropdown">
                            <ul>
                                ...
                                @if ($settings->citiesIsPresent())
                                    <li> <a href="#" id="current-city">{{$lang->get(162)}}<i class="fa fa-chevron-down"></i></a> {{--All--}}
                                        <ul>
...
<div class="gf-product shop-grid-view-product">
    ...
    // city
    //
    let currentCity = JSON.parse(localStorage.getItem("restaurants_currentCity")) || [];
    console.log("currentCity", currentCity);
    if (currentCity.city == null) {
        localStorage.setItem('restaurants_currentCity', JSON.stringify({city: ""}));
        currentCity = JSON.parse(localStorage.getItem("restaurants_currentCity")) || [];
    }
    if (currentCity.city !== "")
    ...
    function saveCity(name){
        console.log(name);
        localStorage.setItem('restaurants_currentCity', JSON.stringify({city: name}));
        let currentCity = JSON.parse(localStorage.getItem("restaurants_currentCity")) || [];
        console.log("currentCity", currentCity);
