...
{{--extras--}}
                    ...
                    @endif
                        ...
                        <div class="cart-buttons mb-20">
                            ...
                            <div class="btn btn-primary save-change-btn mt-0" onClick="addToBasketByIdCount({{$food->id}}, selectedItem2, arrayExtras5);">
...
<script>
    ...
    //
    var arrayExtras5 = [];
    console.log("extrasdata count", "{{count($food->extrasdata)}}");
    @foreach($food->extrasdata as $key => $item)
        console.log("$food->extrasdata", "{{$item->name}}");
        arrayExtras5.push({
            id: {{$item->id}},
            name: "{{$item->name}}",
            image: "{{$item->image}}",
